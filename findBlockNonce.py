...
def mine_block(k, prev_hash, rand_lines):
    ...
    # Convert the transactions into a single byte string
    transactions = "".join(rand_lines).encode('utf-8')

    # Target binary suffix (k trailing zeros)
    target_suffix = "0" * k
    ...
    while True:
        ...
        # Concatenate prev_hash, transactions, and nonce, then compute SHA-256 hash
        hash_input = prev_hash + transactions + nonce_bytes
        hash_result = hashlib.sha256(hash_input).hexdigest()

        # Check if the hash's binary representation has k trailing zeros
