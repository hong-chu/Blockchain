...
def scanBlocks(chain,start_block,end_block,contract_address):
    ...
    if end_block - start_block < 30:
        ...
        with open(eventfile, mode='a', newline='') as file:
            ...
            if file.tell() == 0:
                writer.writerow(['Block Number', 'Timestamp', 'Token', 'Recipient', 'Amount'])
            for event in events:
                ...
                writer.writerow([block_number, timestamp, token, recipient, amount])
                print(f"Logged event: Block {block_number}, Token {token}, Recipient {recipient}, Amount {amount}")
    else:
